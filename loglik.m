...
for n=1:NP
    ...
    U=XC(startrowind:endrowind,1)*transpose(chcoeff(1,1:1)) + chcoeff(1,NCH+1:end).*XC(startrowind:endrowind,2).*frustLV;    % Size of U NALTMAX*NCSMAX X 1
    
    % Prepare the lambda coefficient... Refer to Bhat's paper
    lamda=repmat(chcoeff(1,NCH+1:end),NALTMAX*NCSMAX,1).*XC(startrowind:endrowind,2);        % Size of vector is NALTMAX*NCSMAX X 1
    ...
    for i=1:(NALTMAX*NCSMAX)-1
        for j=i+1:NALTMAX*NCSMAX
            ...
            threshLTsel(1,1)=threshvec(1,LTsel(1,1));
            threshLTsel(1,2)=threshvec(2,LTsel(1,2));
            UTsel=transpose(R*UT);         % Upper threshold of the selected pair. Size of the vector is 1 X 2
            threshUTsel(1,1)=threshvec(1,UTsel(1,1));
            threshUTsel(1,2)=threshvec(2,UTsel(1,2));
